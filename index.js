...
class Boundary {
  ...
  const ghosts = [
    new Ghost ({
      position: {
        x: Boundary.width *7 + Boundary.height / 2,
    ...
    new Ghost ({
      position: {
        x: Boundary.width *7 + Boundary.height / 2,
        y: Boundary.height *9 + Boundary.width / 2
      },
      velocity: {
        x: Ghost.speed,
        y: 0
...
function circlesCollidesWithRectangle ({
  circle,
  rectangle
})  {
...
if(pellets.length === 0) {
  ...
  ghosts.forEach((ghost => {
    ...
    if(JSON.stringify(collisions) !== JSON.stringify(ghost.prevCollisions)) {
      ...
      else if(ghost.velocity.x > 0) ghost.prevCollisions.push('up')
